...
class LossEstimator:
    ...
    def estimate_loss(self):
        ...
        for split, batch_gen in [('train', self.train_batch_gen), ('val', self.val_batch_gen)]:
            ...
            attention_ablation_hits = torch.zeros(self.model.cfg.n_layers, self.model.cfg.d_model, dtype=torch.bool, device=self.device)
            neuron_ablation_hits = torch.zeros(self.model.cfg.n_layers, self.model.cfg.mlp_hidden_size, dtype=torch.bool, device=self.device)
