...
def fast_measure_importance_ablated(
    ...
    masking_token: int = 1,
    threshold: float = 0.8,
    skip_threshold: float = 0,
    skip_interval: int = 5
) -> Tuple[list, float, list, list, int]:
    """Ablated version of fast_measure_importance using HookedTransformer"""
    ...
    # Convert to hooked transformer for ablated activations
    ...
    ablation_hooks = get_ablation_hooks_for_tl(
        output,
        slice(None),  # Apply to all positions
        analyzer.model.config
    )

    # Get ablated activations for all versions at once
    with torch.no_grad():
        with ht_model.hooks(fwd_hooks=ablation_hooks):
            _, cache = ht_model.run_with_cache(masked_prompts)
            
    # Get activations from cache using transformer lens naming
    all_masked_activations = cache[f'blocks.{layer}.mlp.hook_post'][1:, :, neuron]
    ...
    for i, masked_activations in enumerate(all_masked_activations):
        # Get actual drop in activation when masking this token
        masked_max = masked_activations[initial_argmax].cpu().item()
        importance = (1 - (masked_max / initial_max))
        
        str_token = str_tokens[i]
        tokens_and_importances[i] = (str_token, importance)
        
        # Use threshold on raw importance value
        if importance >= threshold and str_token != "<|endoftext|>":
            important_tokens.append(str_token)
            print(f"Important token found: {str_token} (importance: {importance:.3f})")
...
def fast_prune_ablated(
    ...
    """Ablated version of fast_prune using HookedTransformer"""
    
    try:
        print("\n=== Starting Fast Prune (Ablated) ===")
        print(f"Input length: {len(text_input)}")
        print(f"Pivot index: {pivot_index}")
        print(f"Original activation: {original_activation}")
        ...
        try:
            ...
            for i in range(0, len(truncated_texts), batch_size):
                ...
                with torch.no_grad():
                    ...
                    with ht_model.hooks(fwd_hooks=ablation_hooks):
                        _, cache = ht_model.run_with_cache(padded_batch)
