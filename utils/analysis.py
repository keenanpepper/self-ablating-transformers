...
def collect_activating_texts(model, train_config, n_texts=10, n_context=10, activation_threshold=0.5):
    ...
    attention_activating_texts = torch.zeros(model.cfg.n_layers, model.cfg.d_model, n_texts, n_context+1, dtype=torch.long, device=device)
    neuron_activating_texts = torch.zeros(model.cfg.n_layers, model.cfg.d_mlp, n_texts, n_context+1, dtype=torch.long, device=device)

    attention_counts = torch.zeros(model.cfg.n_layers, model.cfg.d_model, dtype=torch.long, device=device)
    neuron_counts = torch.zeros(model.cfg.n_layers, model.cfg.d_mlp, dtype=torch.long, device=device)
